    ...
    case msg of
        ...
        NextPage ->
            let
                ...
                newAnswers =
                    getAnswers Nothing page.items m.qModels

                allAnswers =
                    DI.union newAnswers m.answers

                newShownAnswers =
                    getAnswers (Just allAnswers) page.items m.qModels

                allShownAnswers =
                    DI.union newShownAnswers m.answers
            in
            if m.page == A.length pages then
                ( { m | answers = allShownAnswers }, C.none )

            else if nextPage.skip allShownAnswers then
                update NextPage
                    { m
                        | page = m.page + 1
                        , answers = allShownAnswers
                    }
...
view m =
    let
        ...
        answers =
            DI.union (getAnswers Nothing items m.qModels) m.answers
...
getAnswer : Maybe Answers -> QConfig -> QModel -> Maybe ( String, Answer )
getAnswer ma qc qm =
    case ( qc, qm ) of
        ( CText c, QText m ) ->
            let
...
getAnswers : Maybe Answers -> List QConfig -> List QModel -> Answers
getAnswers ma items qModels =
    LE.zip items qModels
        |> L.filterMap (uncurry <| getAnswer ma)
        |> DI.fromList
