...
minSliderScale : Float
minSliderScale =
    0.4


maxSliderScale : Float
maxSliderScale =
    0.5


maxCprLambdaDeviate : Float
maxCprLambdaDeviate =
    0.1
...
type alias RandomThings =
    { prtcBoardConfigs : A.Array BoardConfig
    , realPttBoardConfigs : A.Array BoardConfig
    , realCprPayoffs : A.Array Payoffs
    , realCprLambdaDeviates : A.Array Float
    , realCprSliderScales : A.Array Float
    , pairingTime : Float
...
randomThings =
    let
        ...
        gen =
            RA.constant RandomThings
                |> RE.andMap (RE.array nPrtcRounds genBoardConfig)
                |> RE.andMap (RE.array (nRealRounds // 2) genBoardConfig)
                |> RE.andMap (RE.array (nRealRounds // 2) genCprPayoffs)
                |> RE.andMap (RE.array (nRealRounds // 2) (RA.float -maxCprLambdaDeviate maxCprLambdaDeviate))
                |> RE.andMap (RE.array (nRealRounds // 2) (RA.float minSliderScale maxSliderScale))
                |> RE.andMap (RA.float minPairingTime maxPairingTime)
                |> RE.andMap (RE.traverse genCprActTime (L.range 1 nRealRounds) |> RA.map A.fromList)
    in
    RA.step gen seed |> Tuple.first
            ...
            , instr =
                Callout
                    ...
                    , text = dedent """
                        ...
                        The higher your total reward is, the higher your
                        bonus will be. [b|Same for ][cpr b|Blue]. Note that your decisions
                        will affect both your bonus and [cpr|Blue]'s bonus.
                        """
                    , targetAnchor = xy 1 0
                    , instrAnchor = xy 0 1
                    }
...
calcParabola { vy, vx, scale } =
    { x1 = vx - 0.5 / 2 * scale
    , y1 = vy - (0.5 / 2) ^ 2 * scale
    , cx = vx + 0.5 / 2 * scale
    , cy = vy + (0.25 - (0.5 / 2) ^ 2) * scale
    , x2 = vx + 1.5 / 2 * scale
    , y2 = vy - (1.5 / 2) ^ 2 * scale
    }
...
calcPayoffs { vy, vx, scale } lambda =
    -- let
    --     { xTf, yTf } =
    --         sliderLocationToTfs location
    -- in
    { opp = max 0 <| vx + lambda / 2 * scale
    , slf = max 0 <| vy - lambda ^ 2 / 4 * scale
    }
...
xToLambda c x =
    ...
    -- in
    clamp -0.5 1.5 <| (x - c.vx) / c.scale * 2
...
defaultBoardConfig =
    ...
    { vy = 0.5 + (1.5 / 2) ^ 2 / 2 * maxSliderScale, vx = 0.5 - 0.5 / 2 * maxSliderScale, scale = maxSliderScale }
...
genCprPayoffs : RA.Generator Payoffs
genCprPayoffs =
    RA.map2 Payoffs
        (RA.float
            (maxSliderScale * (1.5 / 2) ^ 2)
            (1 - maxSliderScale * ((1 + maxCprLambdaDeviate) / 2) ^ 2)
        )
        (RA.float
            (maxSliderScale * (1.5 + maxCprLambdaDeviate) / 2)
            (1 - maxSliderScale * (1.5 + maxCprLambdaDeviate) / 2)
        )


genBoardConfig : RA.Generator BoardConfig
genBoardConfig =
    -- lambda_min \in (-2.38, -0.92)
    -- lambda_max \in (1.75, 2.97)
    RA.constant BoardConfig
        |> RE.andMap (RA.float ((1.5 / 2) ^ 2 * maxSliderScale) 1)
        |> RE.andMap (RA.float (0.5 / 2 * maxSliderScale) (1 - 1.5 / 2 * maxSliderScale))
        |> RE.andMap (RA.float minSliderScale maxSliderScale)
...
genCprActTime : Int -> RA.Generator Float
genCprActTime i =
    let
        minTime =
            if i == 1 then
...
getRealBoardConfig : Float -> Int -> BoardConfig
getRealBoardConfig cprLambda r =
    if modBy 2 r == 1 then
        A.get (r // 2) randomThings.realPttBoardConfigs |> M.withDefault defaultBoardConfig
        ...
        let
            ...
            { slf, opp } =
                A.get i randomThings.realCprPayoffs |> M.withDefault { slf = 0, opp = 0 }

            lambdaDeviate =
                A.get i randomThings.realCprLambdaDeviates |> M.withDefault 0

            lambda =
                cprLambda + lambdaDeviate

            scale =
                A.get i randomThings.realCprSliderScales |> M.withDefault maxSliderScale
        in
        { vy = slf + (lambda / 2) ^ 2 * scale, vx = opp - lambda / 2 * scale, scale = scale }
...
getRealCprLambda : Float -> Int -> Float
getRealCprLambda cprLambda r =
    let
        base =
    ...
    let
        ...
        { boardConfig, cprLambda, oppReceiver, memory, lastRound, cprActTime } =
            if prtcStage m.stage then
                ...
                { boardConfig = getRealBoardConfig p.cprLambda m.round
                , cprLambda = getRealCprLambda p.cprLambda m.round
